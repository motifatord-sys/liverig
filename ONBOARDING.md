...
7. ~~The Setup Tool's own config-sync step didn't actually exist in code~~ — **closed 2026-07-02.** The logic had genuinely existed once, correctly, in `LiveRig_Wired_Start.sh` (a retired predecessor to LiveRig.app) but was never carried forward into the bash-launcher/py2app rewrites — found while investigating this gap. Ported into `liverig-app/liverig_app.py`'s `_sync_rig_config()`: copies a newer `rig_config.json` from the Setup Tool extension's storage directory (`~/Library/Application Support/Ableton/Extensions/LiveRig Setup Tool/storage/`) into the repo on every launch (mtime-compared, so it never overwrites a manually-edited config with something older), plus a "Resync Config from Setup Tool" menu item for picking up a change without a full restart. **Also found and fixed a second, more severe gap while porting this:** nothing was copying the repo's `rig_config.json` into `/private/tmp/` either, which is what the iPad's controller page actually fetches from — meaning the real per-keyboard/stem configuration was never reaching the iPad at all; it was silently falling back to the generic 4-keyboard/8-stem default every single launch, regardless of what `rig_config.json` said. Verified in isolation with a 4-case mtime-logic test (no config anywhere / repo-only / extension older / extension newer) — all passed. Not yet confirmed live with the actual Setup Tool extension and a real `LiveRig.app` launch.
...
Closing any of these is real, scoped work — with #4, #7, and #8 all closed now, #2 (Setup Tool missing looper/aux fields) or #3 (silent fallback on a missing/bad config) are probably next, since those are the remaining spots where something can be silently wrong with no indication on the iPad.
