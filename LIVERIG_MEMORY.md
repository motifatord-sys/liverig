...
**Last updated:** 2026-07-02 (**CLICK/GUIDE moved off the Master page onto the Stems tab as stem9/stem10 — now real name-bound stems (ch6, Remote Script handles vol/mute/solo + bidirectional feedback), dropping their last ch5 Cmd+M dependency; needs a Live 12.4.2 restart + iPad reload to take effect — see dedicated section below**; **Crash-restart mid-show fixed: bridge now requests full state resync from Ableton on every new WebSocket connection, commit `6e8752d` — see dedicated section below**; **Omnisphere patch names confirmed unreadable via Live API (hard platform limit), manual per-fader rename shipped instead — see dedicated section below**; **"LiveRig not connecting on iPad" traced through 3 stacked bugs and fixed — ASCII crash, main-thread alert crash, and inherited PYTHONHOME poisoning every bridge-venv rebuild — see dedicated section below; confirmed working live**; **repo reorganized into docs/ + archive/, README.md rewritten to match current architecture — see dedicated section below**; **rig_config.json sync gap closed — see dedicated section below; this also uncovered and fixed a second, more severe bug where the iPad's controller page was never receiving the real config at all**; **LiveRig.app rebuilt as a real py2app-frozen application, closing the "Python" Dock-icon bug — see dedicated section below; David still needs to run `scripts/build_app.sh` once on his Mac**; **Binding-status validation shipped, all 3 parts — commits `a43eb5d` + `dbdc579`** — see dedicated section below; **LiveRig.app's launcher script now auto-deploys the Remote Script on every launch, closing the deployment gap below for good** — plus a new `scripts/build_dmg.sh` for a proper distributable installer, pending David running it once on his own Mac to re-sign + package; ONBOARDING.md added — full install-to-operation walkthrough + honest gap list; Found and fixed the underlying deployment gap: `LiveRig.py` was stale at the real Ableton install path for most of this session's work, now synced; Bidirectional volume feedback added for KBD/Stems/FX-Returns + Click/Guide automation scaffolded pending real track names, commit `1674e86`; Master/Stems volume faders fixed to control Ableton directly, commit `9a5233d`; Clips page rebuilt for native Live API clip control + accurate Session View state, commit `867820e`; 4 UI fixes — Stems fader sizing, Master LOOPS fader removed, Clips play-icon emoji replaced, Clips real clip names — commit `4954ee8`; Blue Hand mode shipped, commit `727a6c7`; >4 keyboard support shipped, commit `4e971c3`; dead `_dump_looper_params` method deleted, commit `4e971c3`)

## CLICK/GUIDE moved from Master page to Stems tab as name-bound stems — 2026-07-02

David: CLICK and GUIDE should no longer be on the Master page — move them to the Stems tab, replacing the two unused slots Stem 9 and Stem 10. Chose (when asked) to make them **full name-bound stems**, not just relocate the old faders.

**Why this is an upgrade, not just a UI move:** CLICK/GUIDE were the last two faders in the whole rig still on ch5 via a manual Cmd+M mapping (`_dispatch_cc` never handled them; the HTML comment even said "no rig_config binding exists yet for these two aux tracks"). As stems they're now on ch6, bound by track name through the exact same config-driven path every other stem uses — so the Remote Script handles their volume/mute/solo and gives bidirectional feedback (value + real track name + color), with zero Cmd+M mapping. This removes the rig's last Cmd+M dependency.

**Changes:**
- `rig_config.json` — `stem9` → label `CLICK`, trackName `CLICK`; `stem10` → label `GUIDE`, trackName `GUIDE` (were `Stem 9`/`Stem 10` with `trackName: null`). Track-name matching in `LiveRig.py._resolve_track_binding` is **case-insensitive** (`.strip().lower()` both sides), confirmed at line ~826/830, so casing/spacing of the Ableton track names doesn't matter, only spelling. David confirmed the tracks are named "CLICK"/"GUIDE".
- `live_rig_3_controller.html` `buildMaster()` — deleted the two ch5 CLICK/GUIDE entries from the `CHANNELS` array; Master is now 8 columns (KBD1-4 + REV1/REV2/DLY1/DLY2) instead of 10, grid auto-adjusts via `repeat(N,1fr)`. Their `aux-*` CSS + `FB_AUX_VOLUME` (0x55) plumbing left in place but unused (no `"aux"` key in config), harmless.
- **No `LiveRig.py` change needed** — stems are fully config-driven and it already iterates all 10 slots (stem9/10 existed as null-bound). Giving them real names is enough for the Remote Script to bind + feed them back. So no Remote-Script redeploy required either.

**Deployment done:** `rig_config.json` only needs the `~/Desktop/liverig/` copy — `LiveRig.py` reads that path directly (first search path), and `liverig_app.py._sync_rig_config()` copies it to `/private/tmp/rig_config.json` for the iPad on every launch; it's NOT in the app bundle Resources. `live_rig_3_controller.html` copied to `/Applications/LiveRig.app/Contents/Resources/`, diff-verified byte-identical.

**Verified:** JSON parses (stem9=CLICK, stem10=GUIDE, count 10), `node --check` on extracted `<script>` clean, no `ch:5` entries remain in the HTML, bundle HTML byte-identical.

**To take effect on David's rig (NOT yet done live):**
1. Restart **Ableton Live 12.4.2 Suite** (or toggle Control Surface dropdown to None and back) so `LiveRig.py` re-reads the config and binds the CLICK/GUIDE tracks — Remote Scripts only read config at init. Confirm via a fresh "rig_config loaded" line in `~/Library/Preferences/Ableton/Live 12.4.2/Log.txt`.
...
**Root cause:** `LiveRig.py` already had `SX_REQUEST_FULL_STATE = 0x4A`, handled (`elif code == SX_REQUEST_FULL_STATE: self._emit_full_state()`) — but a full codebase grep confirmed nothing anywhere ever sent that code. `_emit_full_state()` (transport, bpm, cues, scenes, selected track, all KBD/stem macros + colors + names + volumes, return/aux volumes, looper states/quants, binding statuses, clip grid — everything the UI needs) only ever fired once, ~2s after Ableton first loads the script.

**Fix:** `liverig_bridge_wired.py`'s `handle_client()` now sends `[0xF0, 0x7D, 0x4A, 0x00, 0xF7]` to Ableton immediately whenever a new WebSocket client connects — bridge restart, iPad reload, or a second iPad joining all now trigger a full resync automatically, no Ableton-side action needed. Deployed to the app bundle, not yet tested live through an actual mid-show crash (logic mirrors the one existing call site in `LiveRig.py`, low risk, but flagging for a real-world confirmation next time LiveRig runs live).

**Byproduct finding, worth knowing:** while diagnosing Omnisphere patch-name support (next section), discovered KBD1 currently has **zero live feedback listeners bound** — Omnisphere is a bare `PluginDevice`, and `_select_macro_params()` only binds macro feedback for Rack-wrapped devices by design. Not a bug, just means KBD1's fader labels won't reflect live plugin parameter changes the way a Rack-based keyboard track would.

## Omnisphere patch names — confirmed unreadable via Live API, manual rename shipped instead — 2026-07-02/03

David asked whether LiveRig could read Omnisphere's internal Live Mode patch names (the 8 named slots shown in its UI) and auto-label the KBD1 faders with them. Rather than guess, ran a one-shot diagnostic (`_dump_device_params_diag`, added then removed from `LiveRig.py`, commits `34e977a`/`14b8f87`) logging every parameter Omnisphere exposes to Ableton's Live Object Model.

**Result, pulled directly from `~/Library/Preferences/Ableton/Live 12.4.5b6/Log.txt`:** Omnisphere registers as a plain `PluginDevice` exposing only generic automation parameters (`Device On`, `Param 192-199`, `Level (p1-p8)`) — no patch/preset names anywhere. This is a hard platform limit: the Live API only sees what a plugin registers as automatable, never a black-box VST/AU's internal preset library. Not fixable from the LiveRig side.

**Shipped instead:** long-press-to-rename on KBD fader labels (`live_rig_3_controller.html`, commit `6922667`) — 650ms long-press → prompt → saved to `localStorage` (`KBD_FADER_NAMES`, keyed per keyboard+fader index, per-browser/per-iPad, not synced through `rig_config.json`). Also bumped the KBD page's plugin-name header to 16px white for legibility (commit `59acefc`), generalized for any plugin, not just Omnisphere.

**Open idea, not committed to:** the "Param 192-199" and "Level (p1-p8)" pairs each had identical values in the dump — worth a live fader-nudge test sometime to check if they map 1:1 to the 8 Live Mode slot levels, which could enable native bidirectional level control for KBD1 instead of the current zero-listener state.

## "LiveRig not connecting on iPad" after the py2app rebuild — diagnosed & fixed 2026-07-02 (commits `2106aee`, `515613e`, `38441dc`, `4d5de4f`)

David reported LiveRig wasn't connecting on the iPad right after the py2app rebuild (see "LiveRig.app rebuilt as a real py2app application" below). This turned out to be **three separate, stacked bugs**, each one hiding the next — worth reading in full if this class of "won't connect" symptom ever comes back.

**Bug 1 — ASCII decode crash reading the controller HTML.** `Path.read_text()`/`write_text()` without an explicit encoding use the process locale's preferred encoding. A GUI app launched via Finder/LaunchServices has no inherited terminal `LANG`/`LC_ALL`, so it fell back to ASCII — which crashed on a UTF-8 character in `live_rig_3_controller.html` partway through `_boot()`. Because `_boot()` runs in a background thread, the uncaught exception printed a traceback to stdout (invisible unless the app is launched by running `/Applications/LiveRig.app/Contents/MacOS/LiveRig` directly in Terminal, which is what surfaced it) and silently killed the thread — menu bar icon stayed up, looking normal, but the bridge/HTTP subprocess never started. Fixed with explicit `encoding="utf-8"` on both the read and paired write.

**Bug 2 — the bridge's own error-reporting crashed too.** Once bug 1 was fixed, the bridge subprocess itself failed to start, and the code path meant to tell the user via `rumps.alert(...)` crashed with `NSInternalInconsistencyException` — AppKit requires all `NSAlert`/`NSWindow` creation on the main thread, and `_boot()` runs in a background thread. This masked the real error a second time. Fixed by wrapping that alert in `PyObjCTools.AppHelper.callAfter(...)` so it runs on the main run loop instead of crashing.

**Bug 3 — the real root cause: every venv rebuild was poisoned by inherited `PYTHONHOME`.** With bugs 1-2 fixed, the actual bridge subprocess error surfaced: `Fatal Python error: Failed to import encodings module`. This looked at first like a broken/stale bridge venv (its own Python failing to boot) — but rebuilding the venv from scratch, twice, made zero difference, and confirmed `/opt/homebrew/bin/python3` works perfectly fine when run directly from Terminal (resolves cleanly to python 3.14.4). The actual cause: this app is itself a frozen py2app bundle, and py2app's bootstrap sets `PYTHONHOME`/`PYTHONPATH` in `os.environ` so its own bundled interpreter can find its own bundled stdlib. `subprocess.Popen`/`subprocess.run` inherit the parent's environment by default, so **every** subprocess this app spawned — including the completely separate Homebrew-based bridge venv, and critically the `python3 -m venv ...` / `pip install ...` calls used to *build* that venv in the first place — inherited those same variables, pointing the wrong-version interpreter at the frozen app's own Python resources instead of its own stdlib. This is why rebuilding the venv never helped: the tool doing the rebuilding was hobbled by the same inherited environment on every single attempt.

Fixed in two commits: first added `_clean_subprocess_env()` (an `os.environ` copy with `PYTHONHOME`/`PYTHONPATH` stripped) and applied it to the bridge subprocess launch, the `http.server` subprocess launch, and the venv health-check (`38441dc`) — this fixed the already-built venv case but missed the venv-*creation* path. Second commit (`4d5de4f`) found and fixed the actual gap: `_run()`, the helper used by `_ensure_bridge_venv()` to run `python3 -m venv` and `pip install`, never got the sanitized environment. Now `_run()` defaults to `_clean_subprocess_env()` for every call unless explicitly overridden, so this can't recur for any future subprocess this app spawns.

**Also added while debugging bug 3, worth keeping:** `_bridge_venv_is_healthy()` — actually runs the venv's own `python -c "import sys"` and checks it boots, rather than just checking `BRIDGE_VENV.exists()`. `_ensure_bridge_venv()` now wipes and rebuilds the venv automatically if this check fails (e.g. after a `brew upgrade python` leaves an old venv pointing at a now-gone interpreter) — this class of breakage should self-heal on next launch instead of needing another debugging session.

**Confirmed fixed live 2026-07-02:** bridge log showed a clean boot (`WebSocket ready on port 8765`, `iPad connected from 10.10.0.241`), and `rig_config.json` served `200` instead of the long-standing `404` seen throughout this debugging session (a separate, now also incidentally-fixed instance of the sync gap from the section below). Full functional test in a real playing session with Ableton still pending — connection layer itself is solid.

**Diagnostic technique worth remembering:** running `/Applications/LiveRig.app/Contents/MacOS/LiveRig` directly in Terminal (instead of double-clicking from Finder) was the single most useful move in this whole session — it's the only way to see stdout/stderr from a GUI app's background-thread crashes, which are otherwise completely invisible.

## Repo reorganized into docs/ + archive/ — 2026-07-02

David asked to organize the project folder and make sure code/docs still knew the correct paths. Root had accumulated a mix of active files, secondary reference docs, and fully superseded artifacts from the project's earlier USB + Max-for-Live architecture, all sitting flat at repo root.

**Moved (via `git mv`, full history preserved — nothing deleted, per policy):**
- Into `docs/` (secondary/reference docs, not part of the day-to-day flow): `BRANCHING_STRATEGY.md`, `EXTENSIONS_SDK_BRIEF.md`, `MACRO_TUTORIAL.md`, `REMOTE_SCRIPT_INSTALL.md`.
- Into `archive/` (fully superseded, kept for history only): `SESSION_BACKUP.md`, `LiveRig_Wired_Start.sh` (the old USB/tkinter/M4L launcher — its config-sync logic was recovered and re-implemented in `liverig_app.py`, see the section above), `LiveRig_Bridge.maxpat`, `liverig_dispatch.js`, `liverig_send.js` (the old Max for Live-based feedback stack, replaced by the Remote Script + SysEx approach), the stray root-level `liverig_state.json` (a vestigial tracked empty file from that same old M4L flow — `liverig_bridge_wired.py` actually watches `/tmp/liverig_state.json`, an unrelated runtime path, not this one), `LiveRig_1.0.dmg`, `LiveRig_RemoteScript.zip`, and both `liverig_backup_*.zip` files.
- Confirmed via grep before moving anything: none of these are functional path-dependencies of `liverig_app.py`, `setup.py`, `extension.ts`, or the build scripts — safe to relocate without touching any code.

**Left at root (still actively referenced by relative/absolute paths in code, moving would only add risk for no benefit):** `rig_config.json`/`.example.json`/`.schema.json`, `live_rig_3_controller.html`, `liverig_bridge_wired.py`, `LiveRig/`, `liverig-app/`, `liverig-setup-tool/`, `scripts/`, `vendor/`, `ONBOARDING.md`, `LIVERIG_MEMORY.md`, `.gitignore`.

**Code fixes that came out of this (not just file moves):**
- `live_rig_3_controller.html`'s `showIPPrompt()` still told users to run `LiveRig_Wired_Start.sh` and enter a USB IP — rewritten to describe the current WiFi + LiveRig.app menu bar flow. A nearby code comment naming the same retired script was also updated.
- `README.md` was a full description of the old USB + Max-for-Live architecture (no mention of LiveRig.app, WiFi, the Remote Script, `rig_config.json`, or the Setup Tool at all) — completely rewritten to match current reality, with a repo-layout table reflecting the new `docs/`/`archive/` split, and pointers to ONBOARDING.md/this file instead of duplicating their content.

**On the deletion question:** two of the `.gitignore`-matched zip/state files (`liverig_backup_20260630.zip`, `LiveRig_RemoteScript.zip`) were already untracked, so they moved with a plain `mv`. One (`liverig_backup_20260422.zip`) was still git-tracked despite matching a `.gitignore` pattern added later — had to `git add -f` its new location to register the move as a rename instead of a silent delete+new-file. Nothing in this repo gets permanently deleted by me; anything that looked obsolete went to `archive/` instead, fully recoverable and still there if David wants to prune it himself later.

**Verified:** re-confirmed all of `liverig-app/setup.py`'s relative data-file paths still resolve post-move, re-ran the JS syntax check on `live_rig_3_controller.html` after the text edits, grepped the whole repo for any remaining bare-filename references to the moved files (found only historical narrative mentions in this file's own past entries and `docs/EXTENSIONS_SDK_BRIEF.md`, which are correctly left as-is since they're dated history, not live pointers). **Not yet verified live** — same as the last two sections, needs an actual `LiveRig.app` build/launch to confirm nothing broke in practice.

## rig_config.json sync gap closed — 2026-07-02

Follow-up to the "Python in the Dock" fix: while rewriting the launcher as `liverig_app.py`, ONBOARDING.md gap #7 got flagged — it described a config-sync step ("the launcher compares mtimes and copies the newer config from the Setup Tool's storage folder") that didn't exist in either the old bash launcher or the new one.

**What I found investigating it:** that logic wasn't imaginary or aspirational — it genuinely existed once, working correctly, in `LiveRig_Wired_Start.sh`, a retired predecessor to `LiveRig.app` from before the menu-bar-app architecture existed. It just never got carried forward when the launch mechanism was rewritten (same *class* of gap as the `LiveRig.py` stale-deploy bug from earlier this session — logic that's correct somewhere, silently dropped on a rewrite). `LiveRig_Wired_Start.sh` also confirmed the real, production path convention for the Setup Tool's storage directory: `~/Library/Application Support/Ableton/Extensions/LiveRig Setup Tool/storage/rig_config.json` — matches both the dev-mode CLI's explicit `--storage-directory` flag and the extension's `manifest.json` `"name": "LiveRig Setup Tool"`, so high confidence this is correct (this detail isn't in Ableton's public docs — the Extensions SDK reference itself is gated behind the Centercode beta signup, so I confirmed it from this repo's own history rather than external documentation).

**Also found while reading that old script, more important than the original ask:** a second, more severe gap in the same area. Nothing was copying the repo's `rig_config.json` into `/private/tmp/` either — and that's the actual file the iPad's controller page fetches (`live_rig_3_controller.html` does a plain relative `XMLHttpRequest` GET for `rig_config.json` against the http.server's document root, `/private/tmp`). Without that copy, the iPad was silently falling back to the hardcoded `RIG_CONFIG_DEFAULT` (4 keyboards, 8 generic stems) on every single launch, regardless of what `rig_config.json` actually said — completely independent of the Setup Tool sync question. This is a real functional bug, not just a nice-to-have: `LiveRig.py` (reading the real file straight off disk inside Ableton) and the iPad page (silently defaulting) could end up computing different stem counts and CC ranges for the same rig.

**Fixed:** ported both pieces into `liverig-app/liverig_app.py` as `_sync_rig_config()`, called at the top of `_boot()` alongside `_deploy_remote_script()`:
1. If the Setup Tool extension's storage copy is newer (mtime-compared) than the repo's `rig_config.json`, copy it over. Extensions can't write directly to the Desktop folder themselves — confirmed by a comment in the retired script noting an earlier direct-write attempt caused repeated macOS Desktop-permission prompts, which is exactly why the handoff goes through this already-trusted process instead. (Deliberately did NOT try to make the extension write directly to the repo again for this reason.)
2. Unconditionally copies the repo's `rig_config.json` into `/private/tmp/rig_config.json` every launch, so the iPad always gets the current real config.
3. Added a "Resync Config from Setup Tool" menu item so a config change can take effect with just an iPad page reload instead of a full LiveRig restart.

**Verified:** syntax/`py_compile` on `liverig_app.py`, plus a standalone 4-case test of the exact mtime-comparison logic (no config anywhere / repo-only, no extension copy / extension copy older than repo / extension copy newer than repo) — all four behaved correctly (correct file ends up served in every case, repo only gets overwritten when the extension copy is genuinely newer). **Not yet verified live** — needs an actual Setup Tool save + `LiveRig.app` launch to confirm end-to-end.

## LiveRig.app rebuilt as a real py2app application — 2026-07-02

**The bug:** David launched LiveRig and the Dock showed a generic Python rocket-ship icon labeled "Python" instead of LiveRig's own name/icon — for the entire time the app was running, not just at launch.

**Root cause:** the old architecture was `/Applications/LiveRig.app/Contents/MacOS/LiveRig`, a bash script, which — after copying files and doing setup — ran `"$PYTHON" "$SUPPORT/liverig_menubar.py" ...` directly as a foreground child process. That child was a raw `venv/bin/python` process, never launched through macOS LaunchServices (no `open -a`), so it had zero app-bundle identity of its own in the Window Server's eyes. Since `liverig_menubar.py` uses `rumps` (PyObjC/AppKit under the hood) to create a menu-bar `NSApplication`, macOS had to show *something* in the Dock for that GUI process — and with no bundle to draw a name/icon from, it fell back to the running executable's own identity: Python's.

**The fix:** rather than trying to patch the Dock icon at runtime (there is a lighter PyObjC-only trick — `NSProcessInfo.setProcessName_` + `NSApplication.setApplicationIconImage_` — that was offered first), David explicitly asked for the full, "real" fix: **freeze the whole thing into a proper py2app application.** New structure:

- **`liverig-app/`** (new, in repo) — the py2app project. `liverig_app.py` merges what used to be split across the bash launcher (bootstrap, Remote Script deploy, hostname detection) and the old root-level `liverig_menubar.py` (the rumps menu-bar UI) into one file, one process, one app identity from launch to quit. `setup.py` is the py2app build config — `iconfile` points at `LiveRig_icon.icns` (copied into the repo from the old bundle's Resources, which was its only copy and had never been committed), and `plist` overrides set `CFBundleName`/`CFBundleDisplayName` = "LiveRig", `LSUIElement: False` (deliberately — this keeps the existing Dock-visible behavior, just correctly branded, rather than newly hiding it).
- **Two dependency tiers, kept deliberately separate** (this was a real design decision, not an oversight): the app itself (rumps/PyObjC) gets frozen into the bundle by py2app at build time — end users install nothing for this part. The MIDI bridge subprocess's dependencies (`python-rtmidi` + `websockets`) stay in a separate, lazily-created venv under `~/Library/Application Support/LiveRig/venv`, installed via Homebrew on first run, exactly as before. Reasoning: `python-rtmidi` wraps a C++ library (RtMidi); installing it fresh per-machine is far more robust than trying to cross-bundle a compiled extension inside a py2app archive.
- **`scripts/build_app.sh`** (new) — David runs this on his own Mac. Creates an isolated `liverig-app/build-venv` (py2app + rumps, separate from the runtime bridge venv so building never touches the running app's dependencies), builds `dist/LiveRig.app`, installs it to `/Applications`. **py2app cannot run in a Claude sandbox** — needs macOS + PyObjC + Xcode Command Line Tools — same class of constraint as `codesign`/`hdiutil` for the DMG. **Not yet run/confirmed by David** — top item until he does this.
- **`scripts/build_dmg.sh`** (existing) — updated to reflect that `/Applications/LiveRig.app` is now py2app output rather than a hand-maintained bundle; still does the same ad-hoc re-sign + DMG packaging, unchanged otherwise. Run `build_app.sh` then `build_dmg.sh`, in that order, whenever `liverig-app/` changes.
- **Retired:** the old bash launcher (never had a repo copy of its own — lived only inside the deployed bundle, now fully superseded) and the root-level `liverig_menubar.py` (deleted from the repo, replaced by `liverig-app/liverig_app.py`).

**Verified:** `ast.parse` + `py_compile` on `liverig_app.py`, `ast.parse` on `setup.py`, `bash -n` on both `build_app.sh` and the updated `build_dmg.sh`, and confirmed all of `setup.py`'s relative data-file paths (`../live_rig_3_controller.html`, `../liverig_bridge_wired.py`, `../LiveRig/LiveRig.py`, `../LiveRig/__init__.py`, `LiveRig_icon.icns`) actually resolve on disk. **Cannot verify further in this sandbox** — no macOS, no PyObjC, no way to actually run py2app or launch the resulting app and look at its Dock icon. David needs to run `scripts/build_app.sh`, launch LiveRig, and confirm the Dock/Cmd-Tab/Force-Quit identity now says "LiveRig" with the right icon.

**Bonus find while rewriting the launcher:** ONBOARDING.md has long described a config-sync step ("the launcher script compares mtimes and copies the newer `rig_config.json` from the Setup Tool's storage folder") that does not exist anywhere in the actual code — not in the old bash launcher, not in the new `liverig_app.py`. This is a real, previously undocumented gap (now gap #7 in ONBOARDING.md's list) — a config saved via the Setup Tool modal currently has to be copied over to `rig_config.json` by hand. Not fixed today; flagged for next time.

**Also fixed in passing:** `liverig_bridge_wired.py`'s docstring still said "Wired USB Edition... iPad connects via USB cable only", stale since LiveRig has run over WiFi for a while — corrected the comment, no behavior change.
...
5. ~~Decide whether to delete superseded `SESSION_BACKUP.md` from repo root~~ — resolved 2026-07-02 as part of a full repo reorg: moved to `archive/SESSION_BACKUP.md` (not deleted — nothing gets permanently deleted from this repo, just relocated). See "Repo reorganized into docs/ + archive/" section below for the full move list.
...
1. Read this file first — it's the live state, not `archive/SESSION_BACKUP.md` (moved there 2026-07-02, kept only for history).
...
7. Continue from "Pending tasks" above — next up is confirming the new Clips control live, then Blue Hand.
