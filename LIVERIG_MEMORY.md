...
~~Add packaged (.ablx) build for LiveRig Setup Tool~~ — done, see Extensions SDK section above (`881a29e`). Not yet confirmed installed/working by David.

0. **Confirm the packaged `.ablx` install works end-to-end**: David runs `npm run package` in `liverig-setup-tool/`, turns off Developer Mode in Live 12 Beta Preferences → Extensions, quits any running `npm start`, drags the resulting `dist/liverig-setup-tool.ablx` into the Extensions area (or onto Live directly), and confirms "Configure LiveRig…" shows up on a track right-click without needing `npm start` running. If this works, update the "How to resume"/install docs to recommend the packaged build as the normal day-to-day way to use the tool, reserving `npm start` for active development only.
